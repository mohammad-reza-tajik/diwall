...
const Pagination = (props) => {
    ...
    const pageHandler = (event, page) => {
        ...
        router.push(router) // to navigate to the same page with updated query parameters

    }

    return (
        <Grid component={"section"} container item xs={12} my={20} justifyContent={"center"} alignItems={"center"}>
            <ToggleButtonGroup size={"large"} sx={{gap: 10, p: 10}} color={"primary"} value={page} exclusive
                               onChange={pageHandler}>

                {props.currentPage !== 1 && <ToggleButton sx={{width: 40, height: 40, fontSize: "1.6rem"}}
                                                          value={1}>{1}</ToggleButton>}
                {props.currentPage !== 1 && <Grid component={"p"} fontSize={16} position={"relative"} top={5}>.......</Grid>}

                {props.hasPreviousPage && <ToggleButton sx={{width: 40, height: 40, fontSize: "1.6rem"}}
